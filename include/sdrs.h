...
typedef struct bw_gain_model_t {
    enum type_e {
        GM_TYPE_UNKNOWN=0,
        GM_TYPE_LIME,
    } type;
...
void sdrrx_disable_buffering(IO_HANDLE h);
...
GAIN_MODEL *lime_rx_gain_model_init(POOL *pool);
int lime_rx_set_gain_model(IO_HANDLE h, GAIN_MODEL *model);
int lime_rx_get_gain_model(IO_HANDLE h, GAIN_MODEL *model);
int lime_rx_get_net_gain(IO_HANDLE h, float *gain);

int lime_rx_gain_inc(GAIN_MODEL *model);
int lime_rx_gain_dec(GAIN_MODEL *model);
